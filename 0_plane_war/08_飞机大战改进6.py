    ...
    def update(self):
        if not self.hited:
            ...
            pygame.draw.rect(self.screen, (255, 0, 0),
                             (self.hit_box[0] + self.blood * 3, self.hit_box[1] - 10, 90 - self.blood * 3, 8))
        # 这里显示被击毁之后的特效
        else:
...
class Boss(Sprite):
    ...
    def update(self):
        ...
        # 血条(头顶的红色背景矩形，即：消耗的血）
        pygame.draw.rect(self.screen, (255, 0, 0),
                         (self.hit_box[0] + self.blood * 3, self.hit_box[1] - 10, 150 - self.blood * 3, 8))
    ...
    def __collide(self):
        ...
        for enemy_num in [self.enemys, self.enemys2, self.enemys3, self.enemys4]:
            ...
            if enemy_num.enemy.blood > 0:
                bullet_enemy_collide = pygame.sprite.groupcollide(enemy_num.enemys_group, self.hero.bullets_group,
                                                                  False, True)
                ...
                bullet_enemy_collide = pygame.sprite.groupcollide(enemy_num.enemys_group, self.hero.bullets_group, True,
                                                                  True)
                if bullet_enemy_collide:
                    enemy_num.hited = True
                    # 每次消灭敌机，让数量+1
            ...
            hero_hero_leval_thing_collide = pygame.sprite.groupcollide(self.heros_group,
                                                                       enemy_num.hero_leval_thing_group, False, True)
            ...
            hero_hero_bomb_add_collide = pygame.sprite.groupcollide(self.heros_group, enemy_num.hero_bomb_group, False,
                                                                    True)
            ...
            if self.hero.blood > 0:
                ...
                bullet_hero_collide = pygame.sprite.groupcollide(self.heros_group, enemy_num.bullet_enemy_group, False,
                                                                 True)
        ...
        if self.boss_show_num >= self.enmeys_die_before_boss:
            ...
            if self.boss.blood > 0:
                boss_hero_bullets_collide = pygame.sprite.groupcollide(self.boss_group, self.hero.bullets_group, False,
                                                                       True)
                if boss_hero_bullets_collide:
                    # 每碰撞一下血量减1
                    self.boss.blood -= 1
            else:
                boss_hero_bullets_collide = pygame.sprite.groupcollide(self.boss_group, self.hero.bullets_group, True,
                                                                       True)
                if boss_hero_bullets_collide:
            ...
            if self.hero.blood > 0:
                # 血量大时不坠毁，仅仅生命值, 将spritecollide改成groupcollide,否则没办法仅仅子弹消失飞机不消失
                boss_bullet_hero_collide = pygame.sprite.groupcollide(self.heros_group, self.boss.boss_bullets_group,
                                                                      False, True)
                if boss_bullet_hero_collide:
                    self.hero.blood -= 1

                boss_big_bullet_hero_collide = pygame.sprite.groupcollide(self.heros_group,
                                                                          self.boss.boss_big_shoot_group, False, True)
                ...
                boss_big_bullet_hero_collide = pygame.sprite.spritecollide(self.hero, self.boss.boss_big_shoot_group,
                                                                           True)
                if boss_big_bullet_hero_collide:
                    self.hero.hero_on_off = False  # 关闭英雄发射子弹
                    self.hero.hited = True  # 触发坠机特效
